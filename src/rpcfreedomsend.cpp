...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in patriotbit.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  stop         - Stop masternode configured in patriotbit.conf\n"
                ...
                "  vote-many    - Vote on a PatriotBit initiative\n"
                "  vote         - Vote on a PatriotBit initiative\n"
                );
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "protocol" && strMode != "full" && strMode != "votes" && strMode != "donation" && strMode != "pose"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a PatriotBit initiative (can be additionally filtered, partial match)\n"
                );
    }
