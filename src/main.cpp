...
#if defined(NDEBUG)
# error "PatriotBit cannot be compiled without assertions."
#endif
...
static const int64_t nStartSubsidy = 17 * COIN;
...
int64_t GetBlockValue(int nBits, int nHeight, int64_t nFees)
{
	...
	else if (nHeight <= 1400) {nSubsidy = 1 * COIN;}  
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if(TestNet()) {
    	if(nHeight <= 8800) ret = blockValue * 50 / 100;
    	else if(nHeight >= 8800) ret = blockValue * 25 / 100;
		return ret;
	}
            
    if (nHeight >= 140000){ret = blockValue * 50 / 100;} 
    else if (nHeight >= 88){ ret = blockValue * 25 / 100;} 
    else{ret = 0;}

    return ret;
}

static const int64_t nTargetTimespan = 120; // 2 Minutes
static const int64_t nTargetSpacing = 120; // 2 Minutes
static const int64_t nInterval = 1; // PatriotBit: retarget every block
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    // current difficulty formula, patriotbit - DarkGravity v3, written by Evan Duffield - evan@patriotbitpay.io 
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (retarget == DIFF_BTC)
        {
            ...
            // PatriotBit: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("patriotbit-scriptch");
