...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singlePatriotBit(name.begin()+1, name.end());
            if (mapArgs.count(singlePatriotBit) == 0)
                mapArgs[singlePatriotBit] = entry.second;
            name = singlePatriotBit;
        }
...
#else
    const char* pszModule = "patriotbit";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PatriotBit
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PatriotBit
    // Mac: ~/Library/Application Support/PatriotBit
    // Unix: ~/.patriotbit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PatriotBit";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "patriotbit.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty patriotbit.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override patriotbit.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "patriotbitd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong PatriotBit will not work properly.");
